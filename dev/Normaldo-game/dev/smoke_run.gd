...
func _test_effects(normaldo: Node, spawner: Node) -> void:
	...
	# ПОЛЕ ГАСИМ ПЕРВЫМ, а ждём — ПОСЛЕ. Раньше было наоборот, и оставался зазор:
	# пока идёт ожидание, разбросанное мэджик боксом ещё летит, случайный удар
	# поднимает `_invincible` на полторы секунды, а сразу за ожиданием тест бьёт
	# наручниками — по неуязвимому. Урона нет, замедления нет, тест падает на
	# исправной игре. Ловилось это раз в несколько прогонов, то есть худшим из
	# возможных способов.
	spawner.clear_items()
	await _wait(2.5)
	normaldo.call("_end_scars")
	normaldo.set("_slow_immune_remaining", 0.0)
	# И на всякий случай снимаем саму неуязвимость: её мог поднять удар, успевший
	# случиться в последний миг перед очисткой поля.
