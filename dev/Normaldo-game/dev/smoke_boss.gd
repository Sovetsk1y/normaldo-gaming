...
func _test_prompt(boss: Node) -> void:
	...
	while Time.get_ticks_msec() / 1000.0 - t0 < cycle * 1.6:
		...
		if f0.texture != up:
			pressed = true
		await process_frame
	# Ход берётся у `_drop`, а не у «DROP». Константы `DROP` в `tap_prompt.gd`
	# нет вовсе: ход считается при сборке как `width * DROP_K` и лежит в `_drop`.
	# Обращение к несуществующему полю в GDScript не роняет тест — оно отдаёт
	# null, то есть 0.0, и условие «уехал больше нуля» проходило при ЛЮБОМ
	# движении, включая полное его отсутствие. Проверка была зелёной и пустой.
	var drop : float = float(root.get("_drop"))
	_check(drop > 0.0, "ход пальца посчитан: %.0f px" % drop)
	_check(low - y0 > drop * 0.7,
		"палец уходит вниз на %.0f px при ходе %.0f" % [low - y0, drop])
