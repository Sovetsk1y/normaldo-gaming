...
func _test_sphere() -> void:
	...
	# Носитель обязан быть ПРОЗРАЧНЫМ: любой кадр, в котором шейдер не применился,
	# должен быть пустым, а не белым квадратом во весь размер сферы. Тот голубой
	# квадрат, что вспыхивал вокруг головы, приходил не отсюда, а из самого
	# шейдера (см. energy_sphere.gdshader), — но носитель тут проверять всё равно
	# стоит: это единственная часть сферы, которую можно спросить без экрана.
