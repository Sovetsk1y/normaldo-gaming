...
func _test_slices() -> void:
	...
	# КАЖДЫЙ уровень либо на полосе, либо на плитке — третьего не дано. Проверка
	# ловит ровно тот промах, из-за которого её и завели: уровень, выпавший из
	# раскладки, показал бы стену чужого уровня и никак бы себя не выдал.
	#
	# Считаем через ОБЪЕДИНЕНИЕ двух списков, а не «полос на один меньше, чем
	# уровней». Плиточных уровней теперь ДВА: канализация по замыслу и свалка —
	# потому что её полосу ещё не рисовали. Прежняя арифметика этого не знала и
	# упала на верных данных.
	var covered : Dictionary = {}
	for l in BG.TILE_LEVELS:
		covered[int(l)] = true
	for l in BG.LEVEL_STRIPS:
		covered[int(l)] = true
	_check(covered.size() == BG.LEVEL_COUNT,
		"каждый уровень на полосе или на плитке: %d при %d уровнях"
...
func _test_scene() -> void:
	...
	# Берём ПЕРВЫЙ полосный уровень из самой раскладки, а не число 2: свалка
	# сидит на плитке, и жёстко вписанная двойка проверяла бы переход
	# «плитка → плитка», то есть не проверяла бы ничего.
	var first_strip : int = int(BG.LEVEL_STRIPS.keys().min())
	bg.call("set_level", first_strip)
	await process_frame
