...
# текущего уровня, а следом, через паузу и заметно быстрее, идёт сплошная стена
# пиццы и нагоняет её на экране.
#
...
#   2. Пауза перед стеной уезжает — либо стена выходит колонне на пятки, либо
#      нагоняет её за краем экрана; в обоих случаях встречи игрок не видит.
...
func _initialize() -> void:
	...
	if pz.is_empty():
		_finish()
		return
	# ПАУЗА ОДНА НА ЛЮБУЮ СКОРОСТЬ, и это главное в ней. Раньше она выводилась
	# из точки встречи, расстояние там постоянное — и пауза падала с ростом
	# скорости: на кампанийных 360 выходило меньше секунды, на хардкорных 430 —
	# 0.82 с. Ровно там, где времени на манёвр меньше всего, стена и наступала
	# колонне на пятки.
	_check(absf(waited - lead) < 0.35,
		"и ровно через паузу: ждали %.2f c при заявленной %.2f" % [waited, lead])

	var pz_speed : float = float(pz[0].get("speed"))
	var mult : float = float(sp.call("_pizza_wall_mult", speed, vp.x))
	_check(absf(pz_speed - speed * mult) < 1.0,
		"и летит быстрее колонны в %.2f раза: %.0f против %.0f"
			% [mult, pz_speed, slow_speed])
	# Множитель ЗАЖАТ С ОБЕИХ СТОРОН: снизу вдвое (иначе не догонит вовсе),
	# сверху 3.2 (иначе пять колонок проскакивают экран раньше, чем игрок
	# успевает переставить голову, и приз превращается в мелькание).
	var mult_ok := true
	for v in [200.0, 250.0, 300.0, 360.0, 430.0]:
		var m : float = float(sp.call("_pizza_wall_mult", v, vp.x))
		if m < float(sp.PIZZA_WALL_SPEED_MULT) - 0.001 \
				or m > float(sp.PIZZA_WALL_MULT_MAX) + 0.001:
			mult_ok = false
	_check(mult_ok, "и на всех скоростях забега множитель в своих границах")
