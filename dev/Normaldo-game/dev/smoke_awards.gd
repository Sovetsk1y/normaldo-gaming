...
func _test_apple() -> void:
	...
	# Выгрузка короче списка РОВНО НА ЧИСЛО ЗАРЕЗЕРВИРОВАННЫХ — а не «просто
	# короче». Прежняя формула `reg.size() < ALL.size()` втихую требовала, чтобы
	# резерв был непустым, и упала в день, когда последнее зарезервированное
	# (`campaign`) дождалось пятого эпизода и завелось по-настоящему. Резерв —
	# состояние временное, тест не должен на нём настаивать.
	var held := 0
	for a in ACH.ALL:
		if bool(a.get("reserved", false)):
			held += 1
	_check(reg.size() == ACH.ALL.size() - held,
		"выгрузка короче списка ровно на резерв: %d из %d при %d в резерве"
		% [reg.size(), ACH.ALL.size(), held])
...
func _test_manager() -> void:
	...
	#
	# Сверяемся со СНИМКОМ, а не с числом 10000. На машине разработчика сейв
	# живой и растёт от прогонов: когда пицц в нём стало 10016, `set_max(10000)`
	# честно ничего не поменял — и проверка упала на верных данных, потому что
	# ждала ровно десять тысяч.
	var was_total : int = int(am.call("value", "pizzas_total"))
	am.call("set_max", "pizzas_total", 5)
	_check(int(am.call("value", "pizzas_total")) == was_total,
		"счётчик не откатывается: %d при %d"
			% [int(am.call("value", "pizzas_total")), was_total])
...
func _test_backend() -> void:
	...
	#
	# Строку берём ВЫДУМАННУЮ, а не настоящую из списка. Раньше тут стояло
	# `by_id("campaign")` — единственное тогда зарезервированное, — и проверка
	# умерла вместе с его резервом. Сегодня зарезервированных нет вовсе, а
	# защита в `_mirror` есть, и проверять надо её, а не наличие подопытного.
	am.call("_mirror", { "id": "нет_такого", "reserved": true,
		"stat": "runs_total", "goal": 1 })
