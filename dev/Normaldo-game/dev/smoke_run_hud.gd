...
func _test_badges(hud: Node, normaldo: Node, save: Node) -> void:
	...
	# Постоянно висят только те кружки, состояние которых МЕНЯЕТСЯ: резисты,
	# активка и венцы 10-го уровня. Всё остальное появляется на время действия.
	#
	# Венцы попадают сюда по тому же признаку, что и резисты: у них ОТКАТ, и
	# вопрос игрока к кружку — «есть он у меня прямо сейчас или ещё копится».
	# Динамический кружок отвечал бы на другой вопрос — «действует ли», — а
	# паучья реакция не действует, она ЖДЁТ.
	for k in static_visible:
		_check(k.begins_with("resist:") or k.begins_with("perk:") or k == "active",
