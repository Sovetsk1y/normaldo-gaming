...
func _test_screens() -> void:
	...
	_check(hud.get("_dev_btn") == null and hud.get("_dev_immortal_btn") == null \
			and hud.get("_dev_boss_btn") == null and hud.get("_dev_phase_btn") == null \
	...
	# кампанию: без кнопок посмотреть на них было нельзя ничем. Теперь крокодил
	# ждёт в конце второго уровня, хозяин клуба — в конце пятого, до обоих
	# доходят игрой, и причина держать их снаружи отпала.
	_check(hud.get("_croc_btn") == null and hud.get("_club_btn") == null,
		"кнопки КРОК и КЛУБ ушли под рубильник вместе со всеми")
