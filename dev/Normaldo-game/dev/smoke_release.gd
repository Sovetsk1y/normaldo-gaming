...
#
# Сейчас пуст, и это правильное состояние. Строка появляется здесь ровно на то
# время, пока настоящего рисунка нет: сейф стоял тут, пока показывался камнем.
const AWAITING_ART : Array = [
	# КОРОБКА ПИЦЦЫ — ключ к мини-игре ПИЦЦА-ПАТИ. Берёт картинку супер-пиццы, и
	# в игре они РАЗЛИЧИМЫ: у ключа лучи, сияние и оранжевый фонтан частиц (общий
	# кирпич minigame_glow.gd), у супер-пиццы ничего этого нет, плюс ключ вдвое
	# крупнее. А вот в КАТАЛОГЕ разницы не остаётся: там обе строки — статичная
	# иконка, и стоят они одна под другой как одно и то же.
	#
	# Своя картинка нужна: та же коробка, но раскрытая или с сиянием, — чтобы
	# каталог отвечал на «а это что» без чтения подписи.
	"pizza_box",
]
