...
func _test_screens() -> void:
	...
	# А вот КРОКОДИЛ обязан ОСТАТЬСЯ. Он не дев-инструмент, а единственный вход в
	# битву: в кампанию босс не встроен, и с выключенным рубильником тестеры без
	# этой кнопки не увидят его вовсе. Проверка стоит здесь именно потому, что
	# следующая же чистка «убрать все кнопки из забега» унесёт его заодно — и
