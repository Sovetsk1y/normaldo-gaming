...
bold "Версия"
# ── ВЕРСИЯ ЖИВЁТ В ИГРЕ ──────────────────────────────────────────────────────
# `--version` правит project.godot, а не пресет. Пресет — производное: его
# перепишет sync_version.py ниже.
#
# Раньше версия лежала в пресете, и это подвело: `export_presets.cfg` у каждого
# свой (в нём настройки подписи), в гите он расходится с рабочей копией — и
# «версия проекта» оказалась величиной, которая у всех разная.
if [[ -n "$NEW_VERSION" ]]; then
	/usr/bin/sed -i '' -E "s|^config/version=.*|config/version=\"${NEW_VERSION}\"|" \
...
# ── СВЕСТИ ВЕРСИЮ И НОМЕР СБОРКИ ─────────────────────────────────────────────
# Версия едет ИЗ ИГРЫ В ПРЕСЕТ (в Info.plist пойдёт она), номер сборки — обратно,
# из пресета в игру (чтобы на экране настроек было видно, какая именно сборка
# стоит на телефоне).
#
# Место выбрано не случайно: ПОСЛЕ bump'а — иначе в игру уедет предыдущий
# номер, — и ДО экспорта, иначе перенос не попадёт ни в .pck, ни в Info.plist.
