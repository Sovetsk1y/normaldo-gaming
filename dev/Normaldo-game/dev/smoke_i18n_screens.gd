...
# Имена скинов сюда больше не входят: у каждого теперь есть английская форма
# («НОРМАЛЬДО» → NORMALDO, «ГааР» → GaaR), и прощать им кириллицу не за что.
# Остались имена, которые в тексте игры звучат сами по себе.
const NAMES : Array = [
	# Название языка пишется НА САМОМ ЯЗЫКЕ: список языков читает тот, кто
	# текущего как раз и не понимает (см. Loc.NAMES).
	"РУССКИЙ"]
# ── СПИСОК ПОБЛАЖЕК — ЭТО СПИСОК ДЫР ───────────────────────────────────────
# Каждая строка здесь означает «на это не смотреть», и одна неверная строка
# выключает проверку на целом куске экрана молча.
#
# Отсюда убраны «СЕТКА» и «КАРТОЧКИ». Это переключатель вида на экране скинов —
# кнопка, которую видит КАЖДЫЙ игрок, а не дев-чип: дев-кнопок с такими именами
# в игре нет вовсе. Пока они лежали тут, английская сборка показывала на экране
# скинов русское слово, и проверка языков об этом молчала.
const DEV_MARKS : Array = ["DEV", "СБРОС", "БОССЫ", "ОСТР", "ОБУЧ", "БЕСС",
	"ВОЛНА", "БОЧКА", "СТЕНА", "МЕШОК", "КРОК", "СВАТ",
...
func _initialize() -> void:
	...
	await _tick(0.6)
	# ── МАРКЕР ОБЯЗАН БЫТЬ СВОИМ У ЭКРАНА ──────────────────────────────────
	# Названия экранов стали рисованными, и по ним больше не опознаться: в
	# надписях их нет. Но дело не только в этом — маркеры «СКИНЫ» и «СЛОТЫ»
	# были негодными И ДО ТОГО: экраны открываются ПОВЕРХ меню, меню остаётся
	# видимым, и оба слова находились на его собственных кнопках. Проверка
	# «экран на месте» проходила бы и с пустым экраном.
	#
	# Теперь маркер у каждого свой и больше нигде не встречается: разделы книги,
	# кнопка карточки скина, таблица призов автомата.
	await _sweep("достижения", "ГЛАВЫ")
	...
	await _tick(0.8)
	await _sweep("скины", "ПОДРОБНЕЕ")

	print("── Слоты ──")
	hud.call("_show_slots")
	await _tick(0.6)
	await _sweep("слоты", "ПРИЗЫ")
