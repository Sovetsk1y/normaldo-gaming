...
def main():
    ...
    w("#")
    w("# `stat` считает scripts/achievement_manager.gd: три примитива и крючки `on_*`,")
    w("# из которых игра дёргает нужный в своей точке события. Кто именно считает")
    w("# каждый счётчик, сверяет dev/smoke_awards.gd — по исходникам, в обе стороны:")
    w("# счётчик без считающего и считающий без счётчика ломаются одинаково тихо.")
    w("#")
    w("# См. /Концепция/Достижения.md, scripts/achievement_manager.gd")
    w("")
