...
func _test_assets() -> void:
	...
	_check(String(lv[lv.size() - 1]["boss"]) == "club",
		"хозяин клуба стоит в конце последнего эпизода")
	# Длину кампании тут НЕ ПРОВЕРЯЕМ. Она уже написана числом в smoke_levels —
	# там ей и место; вторая такая строка ничего не ловит, зато падает от каждой
	# правки кампании, как упала при переходе на пять эпизодов.
	_check(not lv.is_empty(), "кампания не пуста")
...
func _test_police() -> void:
	...
	# ОКНО НАБЛЮДЕНИЯ ДЛИННОЕ И С РАННИМ ВЫХОДОМ. Сторона у каждого приезда
	# выбирается случайно, и на коротком окне «обе стороны» — это подбрасывание
	# монетки: за четырнадцать секунд приездов набиралось немного, и все они
	# могли выпасть с одной стороны. Тест падал на исправной игре примерно раз
	# в три прогона.
	#
	# Выходим, как только собрали ВСЁ, что проверяется ниже, — обе стороны,
	# четыре линии и подмену дыры. Не только «обе стороны»: остальные два факта
	# копятся в этом же цикле, и ранний выход по одному из них оставил бы
	# остальные недособранными, то есть поменял бы одну мигающую проверку на две.
	#
	# ── И ОКНО СЧИТАЕТСЯ ПО ЧАСАМ, А НЕ ПО КАДРАМ ──────────────────────────
	# Первый заход прибавлял к счётчику 1/60 за кадр и называл это секундами. Но
	# акт живёт НА ТАЙМЕРАХ (`create_timer`), то есть по настоящим часам, а
	# headless-прогон идёт не 60 кадров в секунду, а сколько потянет машина: под
	# нагрузкой батареи быстрее, в одиночку медленнее. «Сорок пять секунд»
	# оказывались то тридцатью настоящими, то девятью — и во втором случае из
	# шести залпов успевало пройти три.
	#
	# Три залпа с монеткой на каждый — это каждый восьмой прогон мимо, и ровно
	# так тест и падал: «слева=false, справа=true» на полностью исправной игре.
	# Отдельным прогоном он при этом проходил, что и делало отчёт батареи
	# непонятным.
	var t0 := Time.get_ticks_msec()
	while Time.get_ticks_msec() - t0 < 45000 \
			and not (seen_left and seen_right and swapped and lanes_hit.size() >= 4):
		await _tick(1.0 / 60.0)
...
func _test_fist_hurts() -> void:
	...
	# ЖИР ПРИБИТ К СКИННИ. Он приходит из сейва на диске и от прогона к прогону
	# разный: на УБЕРЕ у Нормальдо четыре жизни, то есть нужны четыре попадания,
	# а между ними полторы секунды неуязвимости и целый цикл «отойти, зарядить,
	# ударить». В такой прогон тест не укладывался и падал — на исправной игре,
	# просто потому что сейв оказался прокачанным.
	#
	# Проверяем «доходит ли рывок», и одного попадания для этого достаточно.
	(e["n"] as Node2D).set("fat_state", 0)
	# И СКИН ПРИБИТ К КЛАССИКЕ — по той же причине, по которой прибит жир, только
	# найдено это позже и дороже. Скин тоже приходит из сейва на диске, а у
	# половины скинов есть пассивка, которая СПАСАЕТ ОТ СМЕРТИ: у Гарри второй
	# шанс, у Дракулы бессмертие, у Джокера шрамы. С таким скином первый
	# смертельный удар тратит пассивку вместо жизни, `_dead` не наступает — и
	# тест падает на исправной игре, сообщая «рывок не доходит», хотя он дошёл.
	#
	# Поймано это было так: тест начал валиться три прогона подряд после того,
	# как СЪЁМКА КАДРОВ лаборатории записала в сейв другой активный скин. То
	# есть тест зависел не от кода, а от того, что запускали до него.
	var save := get_root().get_node_or_null("SaveData")
	if save != null:
		save.set("active_skin", "classic")
	if (e["n"] as Node2D).has_method("reload_skin"):
		(e["n"] as Node2D).call("reload_skin")
	(e["n"] as Node2D).set("_harry_second_chance_ready", false)
	(e["n"] as Node2D).set("_dracula_immortal_ready", false)
	...
	#
	# ЗАПАСА ХВАТАЕТ НА НЕСКОЛЬКО ЦИКЛОВ, и это не перестраховка. Босс подходит,
	# заряжается и бьёт; промахнувшись, он отходит и начинает заново, а момент
	# начала цикла случаен. Прежние восемь секунд вмещали ровно один цикл, и
	# тест падал, когда первый рывок стартовал чуть позже, — то есть на
	# исправной игре, раз в несколько прогонов. Проверяем «доходит ли рывок
	# вообще», а не «успевает ли он в отведённые восемь секунд».
	var t := 0.0
	while t < 20.0 and not bool(e["n"].get("_dead")):
		await _tick(1.0 / 60.0)
		t += 1.0 / 60.0
