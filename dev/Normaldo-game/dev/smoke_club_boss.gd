...
func _test_assets() -> void:
	...
	# дев-кнопкой»: он стоит в конце пятого уровня, как и стоял в старом проекте.
	# Проверяется именно это — кнопка теперь обычный дев-инструмент под общим
	# рубильником, и её наличие ничего про бой не говорит.
	...
	_check(lv.size() == 5, "а уровней в кампании пять: %d" % lv.size())
