...
# Нижняя граница числа проверок — РОВНО столько их сейчас и есть. Стояло 84 при
# фактических 112, и запас в 28 проверок съел целый тест: ошибка в одной строке
# роняла `_test_web_sweep` до первой проверки, три его строки исчезали из отчёта,
# а тест печатал «ВСЁ ЗЕЛЁНОЕ». Граница ловит «не отработало» только если стоит
# впритык.
#
...
# Ждём РЕАЛЬНОЕ время, а не кадры. Считали кадры и делили на 60 — то есть
# держали за 60 кадров в секунду, которых в headless нет: там кадры идут как
# получится, и «1.2 секунды» превращались то в полсекунды игрового времени, то
# в три. Снаряды же летят по настоящей дельте, и проверка «долетел ли» зависела
# от загрузки машины: в одиночку зелено, в общем прогоне — красное.
func _wait(sec: float) -> void:
	var t0 : int = Time.get_ticks_msec()
	while Time.get_ticks_msec() - t0 < int(sec * 1000.0):
		await process_frame

# Сбит ли предмет. Сбитое теперь НЕ ИСЧЕЗАЕТ, а падает вниз и живёт ещё почти
# секунду (см. knock_fall.gd), и подброс у него случайный. Проверка «узла не
# стало» из-за этого была подбрасыванием монетки: успел долететь до низа за
# отведённое время — зелено, не успел — красно, ровно тот же код. Спрашиваем то,
# что и означает «сбит»: либо уже исчез, либо падает.
# Аргумент НЕ типизирован намеренно: в `n` прилетает уже освобождённый узел, а
# проверка типа у типизированного параметра срабатывает ДО тела функции и роняет
# вызов — тест при этом молча не доходит до своих проверок и всё равно печатает
# «ВСЁ ЗЕЛЁНОЕ».
...
func _test_web_sweep() -> void:
	...
	_check(_knocked(bad1) and _knocked(bad2),
