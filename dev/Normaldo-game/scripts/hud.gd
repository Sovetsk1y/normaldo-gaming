...
const MENU_BTN_SETTINGS      := preload("res://assets/ui/menu/btn_settings.png")
const MENU_BTN_BOOK          := preload("res://assets/ui/menu/btn_book.png")
const MENU_BTN_SKINS         := preload("res://assets/ui/menu/btn_skins.png")
const MENU_BTN_SLOTS         := preload("res://assets/ui/menu/btn_slots.png")
const MENU_BTN_QUESTS        := preload("res://assets/ui/menu/btn_quests.png")
const MENU_BTN_LEADERS       := preload("res://assets/ui/menu/btn_leaderboard.png")
const MENU_BTN_SETTINGS_GLOW := preload("res://assets/ui/menu/btn_settings_glow.png")
const MENU_BTN_BOOK_GLOW     := preload("res://assets/ui/menu/btn_book_glow.png")
const MENU_BTN_SKINS_GLOW    := preload("res://assets/ui/menu/btn_skins_glow.png")
const MENU_BTN_SLOTS_GLOW    := preload("res://assets/ui/menu/btn_slots_glow.png")
const MENU_BTN_QUESTS_GLOW   := preload("res://assets/ui/menu/btn_quests_glow.png")
const MENU_BTN_LEADERS_GLOW  := preload("res://assets/ui/menu/btn_leaderboard_glow.png")
...
# Canvas-based menu button.
#
# Each btn_*.png / btn_*_glow.png is a 430×192 menu-canvas overlay with one
# button drawn somewhere in it. This helper:
#   1. Crops the icon to its drawn bbox (so we don't ship the whole 430×192
#      overlay through the renderer per button).
#   2. Crops the glow PNG to (bbox + MENU_GLOW_PAD) so the baked halo extends
#      visibly beyond the button silhouette.
#   3. Places both at `target_pos` (canvas px), scaled to viewport. This
#      "translates" the button from where it was drawn to where the design
#      wants it — no need to redraw the PNG for every layout tweak.
#   4. Lays a 2-line caption on the green plate (sub-region of the bbox) with
#      Russo One + outline/shadow + tightened line spacing.
#   5. Drops an invisible Button on top of the visible button area.
#
# Returns the hit-area Button so callers can wire extras (badges, etc.).
func _build_menu_canvas_btn(
		vp: Vector2,
		icon_tex: Texture2D,
		glow_tex: Texture2D,
		drawn_bbox: Rect2,
		target_pos: Vector2,
		plate_y_in_btn: float,
		plate_h: float,
		caption: String,
		callback: Callable,
		font_size: int = 16,
		# Horizontal nudge of the caption (canvas px). Negative = left, positive
		# = right. Default 0 = caption centred over the button.
		plate_x_offset: float = 0.0
	) -> Button:
	var scale_x : float = vp.x / MENU_CANVAS_W
	var scale_y : float = vp.y / MENU_CANVAS_H
	var icon_w_vp : float = drawn_bbox.size.x * scale_x
	var icon_h_vp : float = drawn_bbox.size.y * scale_y

	# Glow region in source PNG, padded but clamped to the PNG bounds.
	var glow_x0 : float = max(0.0, drawn_bbox.position.x - MENU_GLOW_PAD)
	var glow_y0 : float = max(0.0, drawn_bbox.position.y - MENU_GLOW_PAD)
	var glow_x1 : float = min(MENU_CANVAS_W, drawn_bbox.position.x + drawn_bbox.size.x + MENU_GLOW_PAD)
	var glow_y1 : float = min(MENU_CANVAS_H, drawn_bbox.position.y + drawn_bbox.size.y + MENU_GLOW_PAD)
	var glow_w : float = glow_x1 - glow_x0
	var glow_h : float = glow_y1 - glow_y0
	# Offset of glow top-left relative to icon top-left (negative when the glow
	# extends to the left/above the icon).
	var glow_off_x : float = glow_x0 - drawn_bbox.position.x
	var glow_off_y : float = glow_y0 - drawn_bbox.position.y

	# Visual container: holds glow + icon + caption + badge. Its size matches
	# the icon's display size; its pivot is the icon's centre. Press feedback
	# scales the container so EVERYTHING (icon, glow, caption, badge) shrinks
	# together while the hit-area Button stays the original size for stable
	# input handling.
	var visual_root := Control.new()
	visual_root.size          = Vector2(icon_w_vp, icon_h_vp)
	visual_root.position      = Vector2(target_pos.x * scale_x, target_pos.y * scale_y)
	...
	var glow_atlas := AtlasTexture.new()
	glow_atlas.atlas  = glow_tex
	glow_atlas.region = Rect2(glow_x0, glow_y0, glow_w, glow_h)
	var glow_rect := TextureRect.new()
	glow_rect.texture             = glow_atlas
	glow_rect.stretch_mode        = TextureRect.STRETCH_SCALE
	glow_rect.expand_mode         = TextureRect.EXPAND_IGNORE_SIZE
	glow_rect.custom_minimum_size = Vector2.ZERO
	glow_rect.size                = Vector2(glow_w * scale_x, glow_h * scale_y)
	glow_rect.position            = Vector2(glow_off_x * scale_x, glow_off_y * scale_y)
	glow_rect.texture_filter      = CanvasItem.TEXTURE_FILTER_LINEAR
	glow_rect.mouse_filter        = Control.MOUSE_FILTER_IGNORE
	visual_root.add_child(glow_rect)

	# Icon: crop to exact bbox, NEAREST for crisp pixel art. Sits at (0,0) in
	# visual_root so visual_root's size matches the icon's footprint.
	var icon_atlas := AtlasTexture.new()
	icon_atlas.atlas  = icon_tex
	icon_atlas.region = drawn_bbox
	var icon_rect := TextureRect.new()
	icon_rect.texture             = icon_atlas
	icon_rect.stretch_mode        = TextureRect.STRETCH_SCALE
	icon_rect.expand_mode         = TextureRect.EXPAND_IGNORE_SIZE
	icon_rect.custom_minimum_size = Vector2.ZERO
	icon_rect.size                = Vector2(icon_w_vp, icon_h_vp)
	...
	if caption != "":
		var plate_w : float = drawn_bbox.size.x * scale_x
		var plate_h_vp : float = plate_h * scale_y
		var lbl := Label.new()
		...
		lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
		lbl.vertical_alignment   = VERTICAL_ALIGNMENT_CENTER
		lbl.size                 = Vector2(plate_w, plate_h_vp)
		lbl.position             = Vector2(plate_x_offset * scale_x, plate_y_in_btn * scale_y)
		lbl.mouse_filter         = Control.MOUSE_FILTER_IGNORE
	...
	hit.position   = Vector2(target_pos.x * scale_x, target_pos.y * scale_y)
	hit.pressed.connect(_play_btn_sfx)
	hit.pressed.connect(callback)
	# Stash visual_root so _attach_canvas_badge (and any future helper) can
	# parent extras to it instead of guessing.
	...
	# Cancel the pressed-look if the player drags the touch off the button
	# before releasing it (button_up doesn't fire in that case).
...
func _build_menu_settings_btn(vp: Vector2) -> void:
	# btn_settings.png bbox=(3,16)+(34,27); plate (last 11 px) sits at y=31..42
	# within source → within-bbox y=15..26. Smaller font (12) because the
	# caption "НАСТРОЙКИ" is the longest in the menu and would clip at 16.
	_build_menu_canvas_btn(vp,
		MENU_BTN_SETTINGS, MENU_BTN_SETTINGS_GLOW,
		Rect2(3, 16, 34, 27),
		Vector2(3, 12), 15.0, 13.0,
		"НАСТРОЙКИ",
...
func _build_menu_book_btn(vp: Vector2) -> void:
	...
	var book_btn := _build_menu_canvas_btn(vp,
		MENU_BTN_BOOK, MENU_BTN_BOOK_GLOW,
		Rect2(218, 6, 35, 26),
		Vector2(216, 8), 10.0, 0.0,
		"КНИГА\nУЧИТЕЛЯ",
		_show_achievements)
...
func _build_menu_right_column(vp: Vector2) -> void:
	...
	var skins_btn := _build_menu_canvas_btn(vp,
		MENU_BTN_SKINS, MENU_BTN_SKINS_GLOW,
		Rect2(347, 40, 34, 28),
		Vector2(350, 18), 17.0, 11.0,
		"СКИНЫ", _on_shop_tapped)
	_skins_badge = _attach_canvas_badge(skins_btn, SaveData.has_ready_mastery_chest())

	_build_menu_canvas_btn(vp,
		MENU_BTN_SLOTS, MENU_BTN_SLOTS_GLOW,
		Rect2(347, 99, 34, 37),
		Vector2(350, 45), 26.0, 11.0,
		"СЛОТЫ", _show_slots)

	var quests_btn := _build_menu_canvas_btn(vp,
		MENU_BTN_QUESTS, MENU_BTN_QUESTS_GLOW,
		Rect2(346, 9, 34, 27),
		Vector2(350, 90), 16.0, 11.0,
		"ЗАДАНИЯ", _show_quests)
	_quest_badge = _attach_canvas_badge(quests_btn, QuestManager.has_daily_badge())

	_build_menu_canvas_btn(vp,
		MENU_BTN_LEADERS, MENU_BTN_LEADERS_GLOW,
		Rect2(347, 72, 34, 30),
		Vector2(350, 124), 19.0, 11.0,
