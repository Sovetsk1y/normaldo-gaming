...
# ── ПОРОГИ ЖИРА БЕРУТСЯ У ТОГО, КТО ПО НИМ ТОЛСТЕЕТ ────────────────────────
# Здесь лежала СВОЯ КОПИЯ таблицы — [40, 120, 260], — и она разошлась с
# настоящей ([30, 60, 90] в normaldo.gd). Полоса от этого врала всем и всегда:
# писала «0 / 40», когда до жира оставалось 30, обещала на втором жире 80 пицц
# вместо 30 и на третьем 140 вместо 30. Игрок ел, считал по полосе и толстел
# раньше обещанного — а на верхних жирах полоса замирала почти пустой.
#
# Копия — единственный способ такое получить: числа сравнивает ОДИН код
# (`_eat_pizza`), а показывает ДРУГОЙ, и ничто на свете не заставляет их
# совпадать. Поэтому таблица теперь одна, и живёт она там, где по ней принимают
# решение; сюда она приходит оттуда.
const NORMALDO_SCRIPT := preload("res://scripts/normaldo.gd")
const FAT_THRESHOLDS  := NORMALDO_SCRIPT.FAT_THRESHOLDS
...
func summon_bum_king(test_mode: bool = false) -> void:
	...
	if not normaldo or not game_root:
		return
	var boss := Node2D.new()
	boss.set_script(BUM_KING_SCRIPT)
	boss.call("setup", normaldo, spawner, game_root, test_mode)
	game_root.add_child(boss)
	if not test_mode:
		boss.connect("defeated", _on_boss_defeated)
...
func _summon_boss(kind: String) -> void:
	...
	match kind:
		...
		"bum_king":
			...
			bk.call("setup", normaldo, spawner, game_root, false)
			boss = bk
		_:
