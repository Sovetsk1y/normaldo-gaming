...
# С какого расстояния начинает заряжать. Вдвое больше первоначальных 190: с
# короткой дистанции рывок был почти неотличим от «подошёл вплотную» — заряд
# начинался тогда, когда игрок и так уже отходил, и телеграф работал вхолостую.
# С 380 хозяин заряжает ИЗДАЛЕКА, и рывок становится тем, чем задуман: броском
# через полэкрана, который видно заранее.
const CHARGE_D     : float = 380.0
...
func _finale() -> void:
	...
	# Лист нарисован вверх колёсами: мигалка на нём внизу, а крыша сверху.
	car.flip_v         = true
