...
# `fit_scale` приводит к BASE_PX длинную сторону КАДРА. Это верно ровно до тех
# пор, пока рисунок кадр заполняет. У части ассетов вокруг рисунка нарисованы
# поля, и доходит это до 0.70–0.74 длинной стороны: знак, шляпа мага, маска
# Кейси, наручники. На экране такой предмет выходит на четверть мельче пиццы,
# хотя «по паспорту» у них один размер — ровно это и читается как «многие
# предметы сильно меньше обычных».
#
# Поэтому общий путь предметов (эффекты, опасности, ящик, ниндзя) считает
# размер по СОДЕРЖИМОМУ — `fit_sprite_content`. Рамка кадра при этом перестаёт
# что-либо значить, и новый ассет с полями встаёт в ряд сам.
