...
# ── Порядок кусков: ПИНГ-ПОНГ ────────────────────────────────────────────────
# Куски идут 1 → 2 → … → N → N−1 → … → 1 → 2 → …, то есть полоса доходит до конца
# и отматывается обратно.
#
# Не по кругу — и вот почему. Полоса не нарисована зацикленной: её хвост,
# который короче куска, при нарезке отбрасывается, и последний кусок с первым не
# сходится. Замер стыков: у пятого уровня разрыв 87 из 255 при том, что худший
# СОСЕДСКИЙ стык по всем пяти уровням — 41. То есть цикл давал бы посреди забега
# видимый обрыв стены, а пинг-понг не даёт ни одного: любые два куска, идущие
# подряд, — соседи по исходной полосе, и рисунок у них сходится по построению.
#
# Отмотка назад заметна только там, где на стене есть указатель или надпись, и
# то как «мы вернулись», а не как «картинка порвалась». Из пяти уровней до
# разворота вообще доживают два: остальные полосы длиннее, чем уровень успевает
# проехать.
var _dir : int = 1
