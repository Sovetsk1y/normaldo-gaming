...
#
# См. /Концепция/Уровни/1-Канализация.md, /Концепция/Уровни/Кампания — пять эпизодов.md
...
# ── Полосы по эпизодам ────────────────────────────────────────────────────────
# Эпизодов ПЯТЬ, и фонов ровно пять: плитка канализации и четыре полосы. Один в
# один, без дырок и без пар.
#
# Раньше это сходилось иначе. Эпизодов было три, и полосы шли ПАРАМИ
# (2+3 = «улица», 4+5 = «дорога в клуб»), потому что два места были слеплены в
# одно. Теперь они разлеплены, и каждой полосе досталось своё: река, пляж, двор,
# клуб.
const TILE_LEVEL   : int = 1
const STRIP_SLICES : Dictionary = { 2: 31, 3: 41, 4: 40, 5: 10 }
const LEVEL_STRIPS : Dictionary = { 2: [2], 3: [3], 4: [4], 5: [5] }
const LEVEL_COUNT  : int = 5

# На каких эпизодах стоит плитка. Список, а не одно число: полос может не
# хватить на все эпизоды, и тогда сюда добавится строка, а не появится второй
# способ отвечать на тот же вопрос.
const TILE_LEVELS : Array = [1]
...
# Запасной вариант — ПЕРВАЯ полоса, а не «полоса номер 2»: ключи таблицы
# менялись уже дважды (пять уровней → три → шесть), и захардкоженный номер
# пережил бы не всякую правку. Теперь запасной берётся из самой таблицы.
func _strips() -> Array:
	if LEVEL_STRIPS.has(_level):
		return LEVEL_STRIPS[_level] as Array
	return LEVEL_STRIPS[LEVEL_STRIPS.keys().min()] as Array
