...
# Втрое медленнее предметов, а не вдвое. Стена ехала почти вровень с потоком,
# и глаз читал скорость по НЕЙ, а не по летящему: предметы казались медленными,
# а фон мельтешил. Задник обязан быть заметно медленнее того, что убивает.
const SCROLL_SPEED : float = 68.0
const SLICE_W      : float = 645.0
const SLICE_H      : float = 430.0

# ── Три уровня, пять нарисованных полос ──────────────────────────────────────
# Полос из старого проекта пять, а уровней в кампании ТРИ: полосы 2 и 3 — это
# один и тот же путь по улице, 4 и 5 — один и тот же двор перед клубом. Уровень
# поэтому держит СПИСОК полос и идёт по ним подряд, замыкаясь на первую своего
# списка.
#
# Склейка полос внутри уровня — такой же стык, как замыкание одной полосы на
# себя (см. ниже): полосы не рисовались продолжением друг друга. Но это ОДНО
# помещение, и на нём стык читается как поворот, а не как подмена — в отличие от
# перехода между уровнями, который прячут карточкой.
#
# Куски НЕ ПРЕДЗАГРУЖАЮТСЯ. Сто тридцать текстур 645×430 — это под сто сорок
...
const STRIP_SLICES : Dictionary = { 1: 14, 2: 31, 3: 41, 4: 40, 5: 10 }
const LEVEL_STRIPS : Dictionary = { 1: [1], 2: [2, 3], 3: [4, 5] }
...
# на всех пяти полосах, худший соседский 41. Один плохой стык на круг есть, и
...
func _level_slice_count() -> int:
	var total := 0
	for strip in (LEVEL_STRIPS.get(_level, [1]) as Array):
		total += int(STRIP_SLICES.get(int(strip), 14))
	return maxi(1, total)

func _take_next_slice() -> Texture2D:
	var n : int = _level_slice_count()
	var idx : int = _next_idx
	var strip : int = int((LEVEL_STRIPS.get(_level, [1]) as Array)[0])
	for st in (LEVEL_STRIPS.get(_level, [1]) as Array):
		var cnt : int = int(STRIP_SLICES.get(int(st), 14))
		if idx < cnt:
			strip = int(st)
			break
		idx -= cnt
