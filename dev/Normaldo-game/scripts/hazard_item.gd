...
func _cry() -> void:
	...
	parent.add_child(a)
	a.play()
	# Крик ОБРЕЗАН по времени. Присланная запись чайки длится 13.8 секунды —
	# это полевая запись, а не звук предмета: птица успевает пролететь экран и
	# исчезнуть трижды, а на второй и третьей птице записи накладываются друг на
	# друга и превращаются в птичий базар.
	#
	# Режем в коде, а не в файле: файл авторский, и резать его вслепую (на
	# машине нет ни одного mp3-декодера, послушать нечем) значит рисковать
	# выкинуть сам крик. Здесь же длину видно и можно поменять одной цифрой.
	var tw := a.create_tween()
	tw.tween_interval(CRY_LEN)
	tw.tween_property(a, "volume_db", -40.0, CRY_FADE)   # без фейда на срезе щелчок
