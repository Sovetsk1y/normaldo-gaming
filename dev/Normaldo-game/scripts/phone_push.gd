...
func _build_banner(body: String, sender: String, lead: String = "") -> void:
	...
	# Он лежит отдельным узлом в ТОЙ ЖЕ сцене, но своей опорой: сам узел
	# приклеен к голове (`_process`), а пуш обязан висеть у верхнего края
	# экрана. Внутри едущего родителя он ездил бы вместе с ним.
	...
	var h := _layout(body, snd, lead)
	_banner.size = Vector2(BAN_W, h)
	_banner.position = Vector2(_banner_x(vp), -h - 10.0)
	var tw := _banner.create_tween()
	tw.tween_property(_banner, "position:y", BAN_TOP, BAN_IN)\
		.set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)

# ── БАННЕР ПРИЖАТ ВПРАВО, А НЕ ПО ЦЕНТРУ ───────────────────────────────────
# По центру он ложился НА КРУЖКИ РЕЗИСТОВ: они стоят слева под верхней полосой,
# их число зависит от скина и от прокачки, и у прокачанного ряд дотягивается до
# трети экрана. Иконка отправителя и его имя оказывались ровно под ними — то
# есть пуш приходил без обратного адреса.
#
# Опускать баннер ниже нельзя: он уехал бы в поле, по которому летят предметы.
# Сужать — тоже: текст в две строки и так впритык. Левая треть верха занята
# постоянно, правая пуста весь забег — туда он и встаёт.
#
# Правый край спрашивается у SafeArea: на телефонах с островком тот сидит
# посреди одного из краёв, и прижатый к самому краю баннер уехал бы под него.
const BAN_MARGIN : float = 14.0

# ── ОСТРОВОК СПРАШИВАЕТСЯ ЧЕРЕЗ ДЕРЕВО, А НЕ ПО ИМЕНИ ──────────────────────
# Этот файл — глобальный класс (`class_name PhonePush`), и такие разбираются
# РАНЬШЕ, чем поднимаются автолоады. Написанное здесь `SafeArea.content_right()`
# роняло компиляцию: «Identifier not found: SafeArea» — и телефон не собирался
# вовсе. Той же болезнью болеют обучение и тур по меню; лечится она одинаково —
# брать автолоад в момент работы, а не в момент разбора.
func _banner_x(vp: Vector2) -> float:
	var right : float = vp.x - BAN_MARGIN
	var sa : Node = get_tree().root.get_node_or_null("SafeArea") if is_inside_tree() else null
	if sa != null and sa.has_method("content_right"):
		right = float(sa.call("content_right", right))
	return maxf(BAN_MARGIN, right - BAN_W)
...
func _layout(body: String, snd: Dictionary, lead: String = "") -> float:
	var text_x : float = BAN_PAD * 2.0 + ICON_PX
	var text_w : float = BAN_W - text_x - BAN_PAD
