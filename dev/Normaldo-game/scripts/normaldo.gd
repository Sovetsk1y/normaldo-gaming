	...
	if event is InputEventScreenTouch:
		...
		if t.pressed:
			...
			if _region_max_x >= 0.0 and t.position.x > _region_max_x:
				...
				return
			# ПАЛЕЦ НА ЭКРАНЕ — ЗНАЧИТ СМОТРИМ ВПЕРЁД.
			#
			# Взгляд разворачивается по движению: увёл голову влево — она и
			# смотрит влево. Но разворот ОСТАЁТСЯ таким после того, как палец
			# убрали, и дальше Нормальдо летит затылком к потоку, пока следующий
			# свайп случайно не окажется вправо. Со стороны это выглядит не как
			# «он смотрит назад», а как «спрайт залип отражённым».
			#
			# Новое касание — заявка на движение, и начинается она лицом туда,
			# откуда всё летит. Свайп влево тут же развернёт обратно, если игрок
			# этого и хочет.
			_set_facing(false)
