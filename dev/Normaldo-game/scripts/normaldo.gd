...
func _try_spider_reflex(area: Area2D) -> bool:
	...
	_sphere_flash(1.45, Color(1.00, 0.42, 0.45))
...
func _trigger_resist(tag: String, area: Area2D) -> void:
	...
	# спасла.
	# 1.55 доли рисунка головы, а не 2.3: сфера обязана быть ОБОЛОЧКОЙ вокруг
	# головы, а не полем на пол-экрана. На 2.3 она перекрывала соседние линии, и
	# игрок переставал видеть, куда ему уходить, ровно в тот момент, когда его
	# спасли.
	_sphere_flash(1.55)
...
func _sphere_flash(k: float = 1.55, col: Color = EnergySphere.COL_MAIN) -> void:
	var host := get_parent()
	if host == null:
		return
	_status_off("sphere")
