...
# У классики в кадре только голова, но нарисована со сдвигом влево. Правка
# ручная, в пикселях забега. На ЖИРОБОССЕ она умножается на множитель размера и
# уводит лицо за левый край — там берётся замер.
#
# Константой, а не литералом в теле: это число читает лаборатория скинов
# (`skin_lab.gd`), и своя копия там разъехалась бы с игрой молча — ровно это и
# поймал `dev/smoke_skinlab.gd` на первом же прогоне.
const CLASSIC_HEAD_NUDGE_PX : Vector2 = Vector2(-14.0, 0.0)
...
# ── ВЗГЛЯД МОЖНО ЗАБРАТЬ ─────────────────────────────────────────────────────
# В забеге направление решает движение: увёл голову влево — она смотрит влево.
# На арене бомжей это неверно: там есть противник, он ходит вокруг, и голова,
# смотрящая туда, куда последний раз вели пальцем, дерётся затылком.
#
# Пока замок стоит, свайп и отпускание взгляд не трогают — им распоряжается тот,
# кто замок поставил, через `face_towards`.
var _face_locked : bool = false

func set_face_lock(on: bool) -> void:
	_face_locked = on

# Смотреть на точку по горизонтали. Берёт координату, а не «влево/вправо»: у
# зовущего есть позиция противника, а не готовый знак.
func face_towards(x: float) -> void:
	_set_facing(x < position.x, true)

func _set_facing(left: bool, forced: bool = false) -> void:
	if _face_locked and not forced:
		return
...
func play_fat_morph() -> void:
	if not is_instance_valid(_sprite):
		return
	# ЗЕЛЁНОГО ПЛЮСА ЗДЕСЬ НЕТ, и это не пропажа. Он появился вместе со
	# статус-значками и оказался лишним: смена жира и так самая заметная вещь на
	# экране — Нормальдо крутится, схлопывается в точку и разворачивается уже
	# другим, — а поверх этого вспыхивал ещё и значок. Два сообщения об одном
	# событии, причём значок перекрывал как раз тот кадр, ради которого морф и
	# сделан.
	#
	# Сам морф остался: он про «ты стал другим», а значки — про «на тебе что-то
	# висит», и путать их не надо.
	_morphing = true
	var spr := _sprite
...
func enable_input() -> void:
	...
	#
	# Замок взгляда снимается ЗДЕСЬ, а не только там, где ставился: ставит его
	# арена бомжей, а снимает конец боя, — но погибший на арене до конца боя не
	# доживает, и следующий забег начинался бы с головой, слушающей мёртвого
	# противника.
	_face_locked = false
	_set_facing(false)
...
#
# `enable_input()` выше — это старт забега целиком: она гасит покачивание с
# дивана, разворачивает голову вправо, пересобирает рантайм скина и обнуляет
# накопленное — бонусный опыт, бонус мага, отметку джокера. Всё это верно
# ровно один раз, на старте.
#
# А боссы забирают управление на реплику и возвращают после титра — и звали за
# возвратом ту же функцию. То есть каждый бой стирал игроку заработанный за
# забег бонусный опыт и снимал бонус мага, причём молча: пропажу видно только
# по цифрам в конце.
#
# Здесь возврат и есть возврат: один флаг, ничего больше.
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventScreenTouch:
		...
		if t.pressed:
			...
			# ПАЛЬЦА НА ЭКРАНЕ НЕТ — ЗНАЧИТ СМОТРИМ ВПЕРЁД.
			#
			# Взгляд разворачивается по движению: увёл голову влево — она и
			# смотрит влево. Пока палец на экране, это верно: игрок ведёт голову
			# и видит, куда она летит. Но разворот ОСТАЁТСЯ и после того, как
			# палец убрали, и дальше Нормальдо висит затылком к потоку, пока
			# следующий свайп случайно не окажется вправо. Со стороны это
			# читается не как «он смотрит назад», а как залипший отражённым
			# спрайт.
			#
			# Поэтому возврат — на отпускании последнего пальца, а не на
			# касании: пока ведут, слушаемся движения; отпустили — стоим лицом
			# туда, откуда всё летит.
			if _touch_count == 0:
				_set_facing(false)
...
func _try_spider_reflex(area: Area2D) -> bool:
	...
	# синяя: резист ломает конкретный предмет, реакция уводит от любого, и
	# путать их одним цветом значит терять разницу.
	_sphere_flash(1.2, Color(1.00, 0.42, 0.45))
	_show_floating_text("ПАУЧЬЯ РЕАКЦИЯ!", Color(0.95, 0.25, 0.30))
	_play_skill_sfx(SkinSkills.DODGE)
	return true

# Насколько ЧАСЫ подняты над головой — в долях от размера рисунка головы. Одна
# константа на оба состояния, которые их показывают: замедление мира и
# перевёрнутое управление (гриб, компас, шаман). Картинка у них одна, значит и
# посадка одна — разъехавшись, они читались бы как два разных знака.
const CLOCK_DY : float = 0.62

# Часы над головой на всё время замедления мира. Один вход на оба источника —
...
func _mark_world_slow(sec: float) -> void:
	...
	# ЧАСЫ НАД ГОЛОВОЙ, а не на ней. Значок вдвое шире самой головы, и надетый
	# по центру он закрывал лицо целиком: полторы секунды игрок смотрел на
	# крутящийся циферблат вместо Нормальдо — а замедление это как раз тот
	# момент, когда важно видеть, где голова и куда она едет. Над макушкой знак
	# читается так же, а голову не прячет; так же поднят значок оглушения.
	_status_on("hourglass", 1.7, -_art_px() * CLOCK_DY)
...
func apply_invert(duration: float, badge: String = "compass") -> void:
	...
	#
	# И ПОДНЯТЫ ОНИ ТАК ЖЕ. Значок вдвое шире головы: надетый по центру он
	# закрывает лицо целиком, а под грибом и шаманом висит по нескольку секунд —
	# ровно тогда, когда важнее всего видеть, где голова и куда она едет.
	# Подъём один на оба состояния с часами (см. `CLOCK_DY`), потому что и
	# картинка у них одна.
	_status_on("invert", 1.7, -_art_px() * CLOCK_DY)
...
func _trigger_resist(tag: String, area: Area2D) -> void:
	...
	# Сфера на ГОЛОВЕ, а не на предмете. Резист — это свойство Нормальдо, и
	...
	# жир. Осколки остаются на месте удара, оболочка вспыхивает на том, кого она
	# спасла.
	# 1.25 доли рисунка головы. Начинали с 2.3, и сфера перекрывала соседние
	# линии: игрок переставал видеть, куда ему уходить, ровно в тот момент, когда
	# его спасли. Оболочка обязана облегать голову, а не заменять собой экран.
	_sphere_flash(1.25)
...
func _cast_melee(dir: Vector2, reach: float, show_fist: bool = true,
	...
	if proj == null:
		return
	# УДАР КУЛАКОМ — НЕ СНАРЯД. Технически это тот же узел (зона поражения со
	# скоростью 0, которую твин ведёт по дуге замаха), но в игре он ничего не
	# «выпускает»: кулак остаётся при хозяине.
	#
	# Разница важна для того, кто снаряды ПЕРЕХВАТЫВАЕТ. Собака капитана,
	# «съевшая» зону замаха, отменяла бы викингу удар, до которого сама даже не
	# дотянулась, — и выглядело бы это как проглоченный спелл.
	proj.remove_from_group("skill_shot")
	if show_fist:
...
# ── КОЛОДА ДЖОКЕРА ─────────────────────────────────────────────────────────
# ЧЕТЫРЕ карты КРЕСТОМ от головы, по диагоналям. Они ПРОБИВАЮТ всё плохое, что
# встретят, отбиваются от краёв экрана четыре раза — и после четвёртого не
# гаснут, а идут обратно к Джокеру.
#
# ── ПОЧЕМУ КРЕСТ, А НЕ ВЕЕР ПО ТАПУ ────────────────────────────────────────
# Веер был спеллом «в ту сторону»: игрок выбирал направление и получал три карты
# туда. Крест ничего не спрашивает — он про ПЛОЩАДЬ, а не про линию, и это
# другое обещание: не «расчисти дорогу», а «раскидай вокруг себя». Направление
# тапа тут больше не участвует вовсе, и это осознанно: спелл, который бьёт во
# все четыре стороны, не может ещё и целиться.
#
# ── ПОЧЕМУ ОТСКОКИ, И ПОЧЕМУ ВОЗВРАТ ───────────────────────────────────────
# Карта, гаснущая на первом предмете, живёт полсекунды и попадает туда, куда её
# послали. Отскакивающая живёт секунды и попадает туда, куда её НЕ посылали, —
# то есть работает и тогда, когда игрок уже думает о другом.
#
# Возврат — не украшение. Он ЗАКРЫВАЕТ спелл: карта, погасшая у дальней стены,
# оставляет вопрос «долетела или нет», а вернувшаяся в руку отвечает на него
# сама. Заодно обратная дорога — ещё один проход через поле, и по нему видно,
# что каст ещё не кончился.
const CARD_DIRS : Array = [
	Vector2( 1.0, -1.0), Vector2( 1.0, 1.0),
	Vector2(-1.0,  1.0), Vector2(-1.0, -1.0),
]
const CARD_SPEED   : float = 520.0
const CARD_WALLS   : int   = 4
# Жизнь — СТРАХОВКА, а не срок. Четырёх отскоков и дороги домой хватает секунд
# на пять; десять стоят на случай карты, зажатой в углу, чтобы она не осталась
# на экране до конца забега.
const CARD_LIFE    : float = 10.0

func _cast_card_deck(_dir: Vector2) -> void:
	for d in CARD_DIRS:
		var card := _make_anim_sprite("res://assets/skills/joker/", "card", 9, 46.0)
		var v : Vector2 = (d as Vector2).normalized()
		# WEB_BAD_GROUPS, а не _RYAG_HIT_GROUPS: карты сбивают ПЛОХОЕ. В общем
		# списке лежат ещё пицца и доллары, и крест из четырёх карт, пущенный на
		# полэкрана, выкосил бы игроку всю его же добычу.
		var pc := _spawn_skill_projectile(v, CARD_SPEED, card, 26.0,
			WEB_BAD_GROUPS, _break_handler(), 0.0, CARD_LIFE)
		_arm_frames(pc, card, 20.0)
		if pc != null:
			pc.call("arm_walls", CARD_WALLS, self)
			pc.add_child(_make_skill_trail(Color(0.62, 0.24, 0.95), -v))
...
# Сфера вокруг головы — на резист и на паучью реакцию. Живёт в том же словаре
# знаков головы, что и остальные: её так же надо двигать за головой каждый такт
# и так же снимать при уходе в мини-игру и при смерти.
#
# Своей жизнью при этом распоряжается сама (см. energy_sphere.gd): у неё
# `open_amount` и есть её жизнь, и внешний таймер тут только мешал бы —
# `_status_flash_left` для неё считает то же самое время вторым счётчиком.
func _sphere_flash(k: float = 1.25, col: Color = EnergySphere.COL_MAIN) -> void:
	var host := get_parent()
	if host == null:
		return
	_status_off("sphere")
	var s := EnergySphere.make(_art_px() * k, col)
	host.add_child(s)
	s.set_meta("dy", 0.0)
	_status_fx["sphere"] = s
	_status_sync()
	s.play()
	# Из словаря вычёркиваем по её же часам: сама себя она освободит, а запись о
	# ней осталась бы висеть и мешала следующей сфере встать.
	_status_flash_left["sphere"] = EnergySphere.life()

# Вспышка не включается и не выключается рубильником, а НАДУВАЕТСЯ И СПАДАЕТ:
# звёзды удара и плюс жира вырастают и тают, а не появляются целиком и не
# исчезают целиком. Разница в том, читается ли знак как событие или как
# мигнувшая картинка.
#
# Огибающая общая для всех знаков головы: они все про мгновение, и мигать
# по-разному им незачем. Сфера резиста живёт по своей — у неё `open_amount`,
# который умеет то же самое честнее (см. energy_sphere.gd).
