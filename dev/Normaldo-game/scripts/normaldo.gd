...
#
# По вертикали якорь В ЗАБЕГЕ считается от y = 0, а не от pos.y: вертикаль
...
#
# На ЖИРОБОССЕ — РОВНО НАОБОРОТ. Покачивания там нет, зато весь узел растянут в
# несколько раз, и вертикальное смещение головы растягивается вместе с ним.
# У Кусса на третьем жире центр головы на 28 % кадра выше центра рисунка: при
# множителе ×6 это четыреста пикселей, и голова уезжала за верх экрана целиком —
# «во время мини-игры он стоит где-то сверху». Поэтому под боссом якорь берёт
# настоящий pos.y.
func _recalc_head_anchor(tex: Texture2D, pos: Vector2) -> void:
	var sz  : Vector2 = tex.get_size()
	var off := SkinMetrics.offset_for(SaveData.active_skin, fat_state)
	_head_idle_pos = Vector2(pos.x, pos.y if _fat_boss_active else 0.0)
