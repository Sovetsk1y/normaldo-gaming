...
# ── Статус-эффекты ───────────────────────────────────────────────────────────
# Девять зацикленных анимаций по 16 кадров 64×64 (набор Vivid Motion) на десять
# состояний игры. Одиннадцатое — щит резиста — ушло отсюда целиком: оно теперь
# считается шейдером (см. energy_sphere.gd), а нарисованный пузырь из набора
# рядом с ним читался как значок, а не как оболочка. Это принципиально: эффект, повешенный
...
const ART : Dictionary = {
	...
	"armor": "armor", "charm": "charm", "stun": "stun",
