...
const ENTRIES : Array = [
	...
	{ "id": "money_bag", "section": S_ITEM, "title": "МЕШОК ДЕНЕГ",
	  ...
	  "text": "Тапай его на лету: каждый тап растит мешок и число на нём, поймал — получил столько долларов. Раздутый не горит и ломает всё, во что врежется. У пирата — вдвое щедрее." },
	...
	{ "id": "pizza_box", "section": S_ITEM, "title": "КОРОБКА ПИЦЦЫ",
	  "icon": "res://assets/items/pizza_pack_closed.png", "script": "pizza_box",
	  "text": "Светится оранжевым, как мутаген зелёным. Поймал — началась ПИЦЦА-ПАТИ." },
	...
	{ "id": "safe", "section": S_ENEMY, "title": "СЕЙФ",
	  "icon": "res://assets/items/safe.png", "script": "safe",
	  "text": "Два урона — и тем же ударом ВСКРЫВАЕТСЯ: высыпает доллары, а\nиногда и мешок. Единственная сделка в игре: жир в обмен на деньги." },
	...
	{ "id": "compass", "section": S_ENEMY, "title": "КОМПАС",
	  ...
	  "text": "Сбивает направление на пять секунд: свайп работает наоборот." },
	{ "id": "mirror", "section": S_ENEMY, "title": "ЗЕРКАЛО",
	  "icon": "res://assets/items/mirror.png", "script": "mirror_item",
	  "text": "Отражает мир на пять секунд: фон наоборот, поток идёт слева." },
	{ "id": "mushroom", "section": S_ENEMY, "title": "ГРИБ",
	  "icon": "res://assets/items/mushroom.png", "script": "mushroom_item",
	  "text": "Восемь секунд: цвета наизнанку, шаг медленнее, и всё вокруг пицца." },
	...
	{ "id": "croc_item", "section": S_ENEMY, "title": "КРОКОДИЛ В ПОТОКЕ",
	  "icon": "res://assets/bosses/leatherhead/gun.png", "script": "croc_item",
	  "text": "Тормозит, ведёт стволом и бьёт одиночным. Ствол замер — сходи с линии." },
