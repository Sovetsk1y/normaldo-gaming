...
#
# Платит только УДАР ГОЛОВОЙ. Резист (Бэтмен и Джокер на десятом уровне) сейф
# разбивает без денег: резист и так превращает удар в ничто, и добавь к нему
# выплату — получилась бы кнопка «бесплатные деньги» у двух скинов из
# тринадцати. Бумбокс, молотов и спеллы сейф тоже просто уничтожают: платит
# сделка, а не факт разрушения.
#
...
# ── Вскрытие ──────────────────────────────────────────────────────────────────
# Зовёт `normaldo._handle_obstacle` СРАЗУ ПОСЛЕ того, как урон уже нанесён:
# сначала цена, потом товар. Обратный порядок читался бы как «дали денег и
# зачем-то ударили».
