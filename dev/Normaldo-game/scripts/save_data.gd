...
# ── Лестница опыта скина ─────────────────────────────────────────────────────
# Правило одно: до 2-го уровня — 1000 опыта, и каждый следующий уровень дороже
# предыдущего на 500. То есть 1000, 1500, 2000 … 5000 за десятый.
#
# ЛЕСТНИЦА СЧИТАЕТСЯ, А НЕ ВЫПИСАНА ЧИСЛАМИ. Раньше тут лежал готовый список
# [0, 2400, 5600, 10400 …], и правило, по которому он получен, не лежало нигде —
# при первой же правке баланса пришлось бы пересчитывать десять чисел руками и
# сверять их между собой. Теперь правило записано, а числа из него следуют.
#
# `LEVEL_XP[i]` — СУММАРНЫЙ опыт, нужный чтобы быть на уровне i+1; индексация
# и смысл те же, что были, поэтому все читатели (`hud.gd`, полоски прогресса,
# карточки скинов) остались как есть.
const LEVEL_STEP_FIRST : int = 1000   # шаг с 1-го на 2-й
const LEVEL_STEP_GROW  : int = 500    # насколько каждый следующий шаг дороже
const MAX_SKIN_LEVEL   : int = 10

# Массив ТИПИЗИРОВАННЫЙ — `Array[int]`, а не просто `Array`. Читатели пишут
# `var from_xp := SaveData.LEVEL_XP[level - 1]`, и у нетипизированного массива
# такой вывод типа не сходится: пока лестница была `const`, тип элемента брался
# из литерала, а у обычного поля его взять неоткуда.
static func _build_level_xp() -> Array[int]:
	var out  : Array[int] = [0]
	var step : int        = LEVEL_STEP_FIRST
	for _i in MAX_SKIN_LEVEL - 1:
		out.append(out[out.size() - 1] + step)
		step += LEVEL_STEP_GROW
	return out

# Total XP required to be AT each level (index = level - 1)
var LEVEL_XP : Array[int] = _build_level_xp()
