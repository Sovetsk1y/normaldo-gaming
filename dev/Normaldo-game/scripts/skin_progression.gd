...
# Базово доступны ТРИ первых состояния; 4-е открывает награда уровня 2.
func max_fat_state(skin_id: String, level: int) -> int:
	# Базово доступны ТРИ состояния жира (0..2). Четвёртое открывает награда
	# 2-го уровня — так в присланном ТЗ, и так игрок с первого забега видит
	# полноценную кривую толщины, а не два состояния из четырёх.
	var best := 2
...
const PERK_ICONS : Dictionary = {
	# МАСКА, а не рука. Рука стала лицом СПЕЛЛА: ею нарисован кружок активки, и
	# в неё Спайдер на время спелла превращается сам. Венец носил ту же самую
	# картинку — в ряду кружков рядом стояли две одинаковые руки, и они читались
	# как один кружок, нарисованный дважды.
	#
	# Маска подходит и по смыслу: венец — это «паучье чутьё», удар уходит в
	# пустоту сам.
	"spider_reflex": preload("res://assets/skills/spider_man/mask_icon.png"),
	# РУКА С ЧАСАМИ. До неё тут стояли те же песочные часы, что у предмета:
	# перк делает ровно то же, что предмет, и разные рисунки под одно действие
	# заставляли бы искать разницу, которой нет. Но в ряду забега кружок перка
	# оказывался неотличим от кружка подобранных часов — а это РАЗНЫЕ вещи: один
	# про то, что умеет скин, другой про то, что сейчас действует.
	"time_slow":     preload("res://assets/skills/wizard/clock_hand.png"),
	# ×2 — РОВНО ТО, ЧТО ВЕНЕЦ ДЕЛАЕТ, и ничего сверх. Тут была бы уместна
	# монета или пачка денег, но они уже заняты: доллар — предмет потока, мешок —
	# другой предмет, и кружок с ними читался бы как «подобрал», а не как «умеет».
	# Множитель же в игре больше ничем не подписан, кроме как числом со знаком —
	# тем же, каким подписан бросок добычи мини-игры (x3.png рядом).
	"double_money":  preload("res://assets/skills/x2.png"),
}
