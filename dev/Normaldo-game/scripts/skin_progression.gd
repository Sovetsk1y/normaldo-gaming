...
# Предметы, которых в игре ЕЩЁ НЕТ. Иммунитет к ним честно записан в лестницу
# (так задумано дизайном), но сработать ему пока не на чем — он оживёт сам,
# как только предмет появится и попадёт в свою группу. Список нужен, чтобы
# магазин мог пометить такую награду и не выглядеть обманом.
const DORMANT_ITEMS : Array = ["safe", "cocktail", "cop", "poison", "bird", "helm", "shaman"]
