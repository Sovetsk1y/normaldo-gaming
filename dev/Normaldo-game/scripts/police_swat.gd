...
#             упала, — и на этом всё: дальше он идёт пустой и больше ничего не
#             делает. Опасен он ровно тем, что у него в руках, и когда руки
#             пусты, опасности в нём нет — кроме той полосы, которую он занял.
#
...
# ── СТВОЛ: ЧУТЬ ДЛИННЕЕ ГОЛОВЫ, А НЕ ВДВОЕ ─────────────────────────────────
# M16 — единственное, по чему стрелка отличают от щитоносца на скорости, и
# отличать надо ДО первого выстрела. Поэтому ствол заметный.
#
# Но «заметный» и «вдвое длиннее бойца» — разные вещи. Здесь успело постоять
# 156 при голове 84, то есть 1.86 головы: боец с таким стволом читался не как
# вооружённый человек, а как ствол, к которому сзади приделали голову.
#
# Пропорция взята от той, что была в игре изначально (78 при голове 64 — ровно
# 1.22), плюс пять процентов: 1.28 головы. Это тот же силуэт, который игра уже
# держала, только чуть весомее — видно и приклад, и вынос ствола вперёд, и при
# этом боец остаётся крупнее своего оружия.
const M16_PX    : float = HEAD_PX * 1.28
...
func _face_target() -> void:
	# Отработавший гранатомётчик («spent») тоже не доворачивается: целиться ему
	# нечем, и голова, следящая за игроком, обещала бы выстрел, которого не будет.
	if kind == "shield" or kind == "spent" \
			or not is_instance_valid(target) or not is_instance_valid(_gear):
		return
...
# ── ГРАНАТОМЁТЧИК КИДАЕТ ОДИН РАЗ И БОЛЬШЕ НИЧЕГО НЕ ДЕЛАЕТ ────────────────
# Пока граната в руке, он опасен ПЛОЩАДЬЮ: взрыв накрывает круг, и уходить надо
# от места падения. Кинув — идёт дальше пустой.
#
# Раньше он после броска доставал M16 и превращался в стрелка. Читалось это
# плохо: игрок видел, что боец кинул гранату и, значит, отработал, — а тот вдруг
# начинал стрелять, и появлялась угроза, которой на экране ничто не предвещало.
# В руках у бойца ровно то, что видно, и когда руки пусты, ждать от него нечего.
func _throw_grenade() -> void:
	...
	if not is_instance_valid(self) or _dead or not is_inside_tree():
		return
	# И ИДЁТ ДАЛЬШЕ ПУСТОЙ. Снаряжение убирается совсем: пустые руки — это и есть
	# то, по чему игрок узнаёт отработавшего бойца.
	kind = "spent"
	if is_instance_valid(_gear):
		_gear.queue_free()
	_gear = null
