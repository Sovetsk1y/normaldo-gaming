...
# Бочка нормируется по КАДРУ, а не по рисунку. Оба кадра — 200×200 и нарисованы
# в общей рамке в одном масштабе (замер: тело в обоих 89–90 px шириной), поэтому
# нормировка по кадру сама держит тело одинаковым, а сорванная крышка встаёт над
# ним ровно там, где нарисована.
#
# По РИСУНКУ так не выйдет: у открытого кадра крышка тянет контент вверх (165 px
# против 107), и любая нормировка по нему ужимала бы тело на смене кадра.
const BARREL_FRAME_PX : float = 88.0
...
func _set_barrel_frame(tex: Texture2D) -> void:
	_barrel.texture = tex
	ItemSizing.fit_sprite(_barrel, BARREL_FRAME_PX)
...
func _run() -> void:
	...
	# 2. БОЧКА ПАДАЕТ НА БОК. Одним движением: проседает вниз и заваливается на
	...
	# 3. ОТКРЫВАЕТСЯ — второй и последний кадр бочки. Лежит она уже на боку,
