...
func _ready() -> void:
	...
	# Имя для тех, кто спрашивает предмет, как он называется (`_area_tag` в
	# normaldo.gd). Без него бочка — безымянное «препятствие», и «умер от бочки
	# бомжа» отличить от «умер от мусорки» нечем.
	#
	# Тело бомжа НЕ ИМЕНУЕМ намеренно: тег `bum` включил бы на нём отжор Дракулы
	# и резисты по бомжам, а это правка баланса, а не счётчика.
	_barrel_box.set_meta("item_tag", "bum_barrel")
	_sync_boxes()
