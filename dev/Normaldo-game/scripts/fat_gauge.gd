...
# Тревожный пульс черепа — только там, где жир НАСТОЯЩИЙ, то есть в забеге.
# В карточке скина этот же индикатор показывает не «сколько у меня осталось», а
# «на какое состояние я сейчас смотрю»: там нулевой жир — это просто первая
# картинка из четырёх, и мигающий красным череп в витрине читался бы как
# поломка. См. hud._build_skin_card().
var warn_on_empty : bool = true
...
func _paint() -> void:
	for i in _lamps.size():
		...
		l.locked = i > _max
		l.queue_redraw()
	_danger_pulse(warn_on_empty and _fat == 0)
