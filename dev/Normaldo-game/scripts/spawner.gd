...
# ── ПЯТЬ ЭПИЗОДОВ ─────────────────────────────────────────────────────────────
# Столько же, сколько НАРИСОВАННЫХ ФОНОВ: плитка канализации и четыре полосы.
# Названия сняты с самих полос, а не придуманы: 2 — ночная вода с плавниками,
# 3 — песок, зонты и пальмы, 4 — тачки у мастерской, 5 — вывеска NIGHTCLUB и
# байки у входа.
#
# Было три, а до них пять. Тройка появилась, когда локации слепили попарно:
# «УЛИЦА» = река + пляж, «ДОРОГА В КЛУБ» = двор + клуб. Слепленные, они давали
# три эпизода по три минуты; разлепленные — пять по полторы, и это ближе к тому,
...
# `dev/smoke_levels.gd` считает, сколько эпизодов ещё ждут своего.
func boss_is_temp(idx: int) -> bool:
	var l : Dictionary = CAMPAIGN_LEVELS[clampi(idx, 0, CAMPAIGN_LEVELS.size() - 1)]
	return bool(l.get("boss_tmp", false))
...
func _finish_level() -> void:
	...
	#   БЕСКОНЕЧНЫЙ — только после ПОСЛЕДНЕГО из CAMPAIGN_LEVELS: локации
	#   кончились, и дальше идёт хвост (см. `enter_hardcore`).
	# Здесь стояло «только после ТРЕТЬЕГО» — от времён, когда уровней было три.
	# Код всё это время считал по длине таблицы и был прав; врал комментарий, и
	# врал ровно в том месте, куда смотрят, спрашивая «а этот босс в бесконечном
	# есть?». Число уровней в тексте не пишем вовсе — таблица меняется.
...
func _start_level() -> void:
	# ЗАЧИСТКА ПЕРВОЙ, флаги после неё. Порядок тут не вкусовой: `clear_items()`
	# сам ставит `_frozen = true` и глушит процесс, и когда он стоял в конце, он
	# затирал только что снятую заморозку. Уровень начинался ЗАМОРОЖЕННЫМ, и
	# поток молчал до первой буквы NORMALDO — то есть четырнадцать секунд пустого
	# экрана после победы над боссом. Ровно это и читалось как «после надписи WIN
	# большой пробел без предметов аж до первой буквы».
	clear_items()
	_letter_idx    = 0
	_letter_active = false
	_letter_timer  = _letter_period()
...
# Каждый следующий эпизод может выпустить босса из ПРОЙДЕННЫХ — уменьшенным и с
# ОДНИМ его ходом. Это не бой, а цитата боя: бой был один раз и запомнился,
# цитата напоминает, что это за существо, и требует того же умения.
#
#   эпизод 2 — ниндзя
#   эпизод 3 — ниндзя, крокодил
#   эпизод 4 — ниндзя, крокодил
#   эпизод 5 — ниндзя, крокодил (+ полицейский, когда он появится)
#
# ── СТАРОГО ПИРАТА ЗДЕСЬ НЕТ, И НЕ БУДЕТ ────────────────────────────────────
# Не потому, что его забыли. Все остальные боссы дерутся В КАДРЕ ЗАБЕГА: ниндзя
# бросает сюрикены, крокодил стреляет — их ход вырезается и вставляется в поток
# как есть. Пират же разворачивает АРЕНУ: экран становится рингом, вокруг встаёт
# толпа, сверху вешаются полосы ХП, а бой идёт на двойное нажатие. Вынуть оттуда
# «один ход» нельзя — там нет ни одного хода, который читался бы без ринга.
#
# ── ВЕС МЕНЬШЕ, ЧЕМ У ОБЫЧНОЙ УГРОЗЫ ────────────────────────────────────────
# 5–6 против 10–16. Босс в потоке — это сет-пис на три секунды: он тормозит,
# целится и бьёт, и всё это время лейн под ним занят. Два таких подряд
# превращают забег в очередь из мини-сцен, а редкий — в событие.
#
# Крокодил приходит на эпизод позже ниндзя, потому что и боссом он был позже:
# встретить в потоке того, с кем ещё не дрался, значит увидеть непонятную
# фигуру, которая почему-то останавливается посреди экрана.
#
# В БЕСКОНЕЧНОМ ЭТО ТОЖЕ ВЕРНО, и проверять там нечего: бесконечный открывается
# только после ВСЕХ эпизодов (`QuestManager.is_endless_unlocked`), то есть к
# первому же его забегу игрок подрался со всеми. Отдельная развилка «в
# бесконечном боссов в поток не пускать» выглядела бы осторожностью, а была бы
# лишним состоянием.
const HAZ_LEVEL : Array = [
	# 1. КАНАЛИЗАЦИЯ — бочки, камни, бомжи, конусы, банан, дорожный знак.
	# Кобра — своя фауна канализации, отсюда она и не уходит.
	#
	# ЯД УБРАН ИЗ ПОТОКА по просьбе автора: склянки с ядом в новой раскладке нет.
	# Предмет, его спрайт и запись в каталоге остались — убрана только строка
	# веса, то есть вернуть его обратно это одна строка. Цена: резист «яд» у
	# ВОЛШЕБНИКА (6-й уровень скина) стал наградой ни за что — сопротивляться
	# нечему, пока яд не летает.
	{ "banana": 26, "trash": 16, "stone": 14, "homeless": 16, "cone": 12,
	  "roadsign": 10, "snake": 14 },
	# 2. РЕКА — вода и то, что по ней плывёт: штурвал, бутылка с письмом, компас,
	...
	{ "umbrella": 16, "lounger": 14, "bird": 14, "cocktail": 10, "shaman": 8,
	  "banana": 10, "stone": 8, "cone": 8, "ninja": 6, "croc": 5 },
	# 4. ДВОР — машины и те, кто их сторожит: тачка копов, колесо, молотов,
	# собака, бандит, сейф. Отсюда начинается город, и он недружелюбный.
	{ "police_car": 12, "tire": 14, "molotov": 12, "dog": 12, "thief": 12,
	  "homeless": 10, "safe": 8, "trash": 10, "cone": 10, "ninja": 6,
	  "croc": 6 },
	# 5. КЛУБ — вход и всё, что у входа: полицейский, наручники,
	# девочка-зазывала, коктейль, чёрный туз, чек лузера. Молотов и сейф тянутся
...
const HAZARD_FROM_PHASE : Dictionary = {
	"cocktail": 0, "bird": 1, "helm": 2, "safe": 2, "cop": 3, "shaman": 3,
}
...
func _spawn_girl(y: float, vp_w: float, speed: float) -> void:
	...
	# и отличает её от того, что стоит хватать.
	StatusFx.attach(g, "charm", GIRL_PX * 1.15, Vector2.ZERO, 12)
