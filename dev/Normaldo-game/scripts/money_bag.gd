...
# ── ПОТОЛКА НЕТ ────────────────────────────────────────────────────────────
# Ни на числе, ни на выплате, ни на размере: сколько успел настучать, столько и
# получил. Раньше стоял потолок в 2.30 (около пятнадцати тапов), и он же был
# потолком заработка — а мешок и заводился ради того, чтобы за него РАБОТАТЬ.
#
# ── НО ПРИБАВКА УБЫВАЕТ ────────────────────────────────────────────────────
# Линейный рост без потолка ломает не баланс, а экран: при 60 тапах мешок был бы
# 317 пикселей, при 90 — 450, то есть выше самого экрана (430). Такой предмет
# перекрывает все пять полос сразу и подбирает всё вокруг себя радиусом в
# полэкрана — это уже не предмет, а задник.
#
# Поэтому прибавка делится на текущий размер: первые тапы растят как раньше,
# дальше всё скупее. Рост не кончается никогда — он просто становится всё
# медленнее, и мешок остаётся предметом при любом числе тапов:
#
#   10 тапов → ×1.69 (88 px)     40 тапов → ×2.96 (154 px)
#   25 тапов → ×2.40 (125 px)    90 тапов → ×4.36 (227 px)
#
# Само же число на боку и выплата растут РОВНО ПО ЕДИНИЦЕ за тап, без всякого
# убывания: игрок платит вниманием линейно и получать должен линейно.
const GROW_PER_TAP : float = 0.085
# Насколько быстро выдыхается прибавка. Меньше — раньше начинает скупиться.
const GROW_SOFT    : float = 1.3
...
func tap() -> void:
	if _spent:
		return
	# Число и выплата — РОВНО ПО ЕДИНИЦЕ, без убывания. Убывает только рисунок.
	_taps += 1
	_grow += GROW_PER_TAP * GROW_SOFT / (GROW_SOFT + (_grow - 1.0))
...
# ── ЧТО ТЕПЕРЬ КОНЧАЕТ ВЗАИМОДЕЙСТВИЕ ──────────────────────────────────────
# Раньше это делал потолок роста. Теперь — только САМ МЕШОК: он едет через экран
# и уезжает за край. Замедление растягивает это окно (на ×0.34 переезд занимает
# около тринадцати секунд вместо четырёх), но не отменяет — и потому «тапай
# сколько успеешь» остаётся ограниченным, а не бесконечным.
#
# Оттого же и нет дыры «стучу вечно, поток стоит»: мешок уедет и заберёт
# замедление с собой (см. `_exit_tree`).
