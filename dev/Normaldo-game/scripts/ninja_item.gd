...
# Ниндзя крупнее рядового предмета — он «персонаж», а не мусор в трубе. Но
# ненамного: на 96 px он спорил размером с боссом, хотя это рядовой залётный
# гость, и на телефоне занимал заметную долю полосы.
const NINJA_PX : float = 72.0
...
# Предмет, который стоит завесить: летящий, ещё в кадре и СПРАВА от ниндзя —
# то есть тот, который игроку ЕЩЁ ПРЕДСТОИТ прочитать.
#
# Раньше он кидал влево, себе за спину: там летит то, что игрок уже разглядел и
# на что уже принял решение. Завесить это — значит спрятать прошлое; жёлтый
# ниндзя отвечает на вопрос «куда мне теперь нельзя», а вопрос этот всегда про
# то, что впереди.
#
# Сам ниндзя, чужой дым и снаряды в счёт не идут.
func _pick_cover_target(host: Node, vp: Vector2) -> Node2D:
	...
	for c in host.get_children():
		...
		# Справа от ниндзя и не дальше края: предмет, ещё не влетевший в кадр,
		# завешивать бессмысленно — облако сядет в пустоту.
		if p.x < position.x or p.x > vp.x + 40.0:
			continue
		found.append(c)
	if found.is_empty():
		return null
