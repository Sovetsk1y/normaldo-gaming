...
#
# ── ЧЕТЫРЕ АКТА, И КАЖДЫЙ ДОБАВЛЯЕТ, А НЕ МЕНЯЕТ ───────────────────────────
#
#   Акт 1 «СОБАКА»   — «читай геометрию». Одна собака, до восьми отскоков от
...
#                      экзамен по трём предыдущим.
#
# Нарастание тут не в числах, а в ВОПРОСАХ: сначала один, потом два, потом три
# сразу. Акт, который просто добавлял бы врагов, читался бы как «то же самое, но
# гуще».
#
# ── ФИНАЛ ──────────────────────────────────────────────────────────────────
...
func _send_dog() -> Signal:
	...
	return dog.get("came_home") as Signal
...
func _act_squad() -> void:
	...
	if not _alive():
		return
	await get_tree().create_timer(SQUAD_LIVE_T).timeout
...
func _act_strafe() -> void:
	...
	for run in STRAFE_RUNS:
		...
		if not _alive():
			return
		# Между заходами МОЖЕТ сыпаться отряд — но только если прошлый кончился
		# (см. `_squad_clear`). Пока на арене хоть один боец, работает вертолёт:
		# горящая полоса и шеренга свата одновременно — это не «сложнее», это
		# «некуда деться».
		#
		# А если отряд всё-таки идёт, он САДИТСЯ МИМО ГОРЯЩЕЙ ПОЛОСЫ: боец,
		# приземлившийся в огонь, сгорел бы у игрока на глазах, и вся угроза от
		# штурмовки прочиталась бы как «она бьёт и своих».
		if _squad_clear():
...
func _burn_out(made: Array) -> void:
	...
	for i in made.size():
		...
		tw.tween_property(f, "scale", Vector2.ZERO, FIRE_FADE_T / float(made.size()))
		tw.tween_callback((f as Node).queue_free)
		await get_tree().create_timer(FIRE_FADE_T / float(made.size())).timeout
...
func _act_all() -> void:
	...
	if not _alive():
		return
	_send_dog()
	# И здесь то же правило: экзамен — это собака плюс огонь плюс отряд, а не
	# собака плюс огонь плюс ВТОРОЙ отряд поверх первого.
	if _squad_clear():
		await _heli_drop([lane])
		if not _alive():
			return
