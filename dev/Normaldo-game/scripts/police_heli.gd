...
# Турель заметно мельче кабины: пулемёт в полкорпуса читался не как оружие на
# подвесе, а как второй летательный аппарат, прицепленный снизу.
const GUN_PX   : float = 78.0
...
# ── КУДА СМОТРИТ СТВОЛ НА НЕПОВЁРНУТОМ РИСУНКЕ ─────────────────────────────
# Вектор от казённика (он же точка крепления, он же центр вращения) к срезам
# стволов, в долях GUN_PX. Из него считается И наводка, И место дула: если
# держать их порознь, они разъедутся при первом же повороте — ровно это и было
# видно на экране, когда очередь уезжала, а ствол оставался смотреть в одну
# точку.
const BARREL_VEC : Vector2 = Vector2(-0.86, 0.78)
...
# ── НАВОДКА ────────────────────────────────────────────────────────────────
# Турель ПОВОРАЧИВАЕТСЯ к точке. Вращается она вокруг казённика — узел сидит
# именно там (это и делает `anchor_sprite`), поэтому ствол ходит как на подвесе,
# а не крутится вокруг своей середины.
#
# Наводить обязательно ДО выстрела, а не после: очередь рисуется от дула, и если
# ствол ещё смотрит в старую точку, линия выходит из воздуха рядом с ним.
func aim_at(world_point: Vector2) -> void:
	if not is_instance_valid(_gun):
		return
	var d := world_point - _gun.global_position
	if d.length() < 1.0:
		return
	_gun.rotation = d.angle() - BARREL_VEC.angle()

# Мировая точка дула — оттуда и рисуются очереди. Тот же вектор, что и в
# наводке, повёрнутый на текущий угол турели: только так линия очереди выходит
# ПРОДОЛЖЕНИЕМ ствола, а не отдельной палкой рядом.
func muzzle() -> Vector2:
	if is_instance_valid(_gun):
		return _gun.global_position + (BARREL_VEC * GUN_PX).rotated(_gun.rotation)
