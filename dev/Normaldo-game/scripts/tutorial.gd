...
const BEATS : Array = [
	# ── СРОК СТРАХОВКИ — НЕ ЗАПАС «НА ВСЯКИЙ», А ПОТОЛОК ЗАВИСАНИЯ ──────────
	# Такт кончается действием, а срок нужен ровно на случай, когда действия не
	# будет: игрок промахнулся мимо всех пицц, банан улетел не в него, часы он не
	# захотел ловить. Всё это время на экране висит одна и та же надпись и не
	# происходит ничего — и чем длиннее срок, тем дольше это «ничего».
	#
	# Здесь стояли двадцать две секунды на такте про вес. Двадцать две секунды
	# неподвижной подсказки читаются не как «подожди», а как «игра сломалась»:
	# ровно с этим и пришли.
	#
	# Срок считается НЕ ОТ НАЧАЛА ТАКТА, а от конца подачи: `_play` сперва
	# дожидается `_serve`, и только потом заводит эти часы. То есть это ответ на
	# вопрос «сколько ждать, когда выдавать больше нечего», и ответ у него
	# короткий — предмету через весь экран нужно около шести секунд, а такту про
	# вес, где подача сама длится четыре, не нужно почти ничего.
	{ "id": "move",   "big": "ВЕДИ ПАЛЬЦЕМ",       "small": "голова идёт следом",      "limit": 20.0 },
	{ "id": "eat",    "big": "ЕШЬ ПИЦЦУ",          "small": "это очки и вес",          "limit": 8.0 },
	{ "id": "dodge",  "big": "МУСОР — ОБЛЕТАЙ",    "small": "в него нельзя",           "limit": 14.0 },
	{ "id": "fat",    "big": "ВЕС — ЭТО ЖИЗНИ",    "small": "полоса слева: удар съедает одну", "limit": 4.0 },
	{ "id": "slow",   "big": "БЬЁТ НЕ ВСЁ",        "small": "банан не ударит, но замедлит", "limit": 11.0 },
	{ "id": "bonus",  "big": "А ЭТО — БОНУС",      "small": "часы замедлят мир, хватай", "limit": 11.0 },
	{ "id": "dollar", "big": "СОБИРАЙ ДОЛЛАРЫ",    "small": "на них можно купить крутые скины", "limit": 10.0 },
	{ "id": "skill",  "big": "ДВОЙНОЙ ТАП",        "small": "это способность скина",   "limit": 14.0 },
...
# Подсказка. Тип НЕ Control: теперь это узел телефона с пушем (Node2D), а не
# облачко из Control'ов.
var _caption : Node        = null
...
func _serve(id: String) -> void:
	match id:
		...
		"fat":
			...
			_ring_fat_panel()
			# ПАНЕЛЬ ПОКАЗЫВАЕТСЯ ОДНА. Пицца пойдёт после того, как затемнение
			# отработает своё: выданная сразу, она уводит взгляд с той самой
			# панели, ради которой экран и погасили, — а такт кончается по съеденной
			# пицце, то есть подсветку игрок сам себе и обрывает.
			await _breath(DIM_IN + DIM_HOLD)
			for i in 5:
				if not _alive():
					return
...
func _cond_for(id: String) -> Callable:
	match id:
		...
		"fat":
			# ЭТОТ ТАКТ НИЧЕГО НЕ ТРЕБУЕТ, он показывает — где написан вес. Условие
			# тут только затем, чтобы не держать игрока, который всё понял.
			#
			# Раньше оно требовало НАБРАТЬ ВЕС или съесть все пять выданных пицц.
			# И то и другое ложь: первое состояние жира стоит сорока пицц, столько
			# такт не выдаёт вовсе, а промахнуться мимо одной из пяти — обычное
			# дело. Всё, что оставалось, — досидеть срок до конца.
			return func() -> bool:
				return int(_normaldo.get("fat_state")) >= 1 or _ate >= 3
		"slow":
...
#
# Само число теперь живёт у пуша (`PhonePush.BAN_TOP`): место одно и то же,
# решает его тот, кто рисует. Здесь остаётся только правило, по которому оно
# выбрано, — чтобы следующий, кому захочется поднять баннер повыше, знал, во
# что упрётся.
#
# ── ПУШ НА ТЕЛЕФОНЕ, а не облачко из долларов ───────────────────────────────
# Тем же самым, чем игра говорит сюжет: Нормальдо достаёт телефон, сверху падает
# уведомление от Хлеба. Одна дверь на весь разговор с игроком — см.
# `phone_push.gd`.
#
# До этого подсказка приезжала облаком из долларов. Как поверхность для текста
# оно работало, но не отвечало на вопрос, КТО учит: команды висели в воздухе.
# У пуша есть отправитель — и обучение превращается в человека, который пишет.
#
# ── И ВИСИТ ОН СТОЛЬКО, СКОЛЬКО НУЖНО ТАКТУ ────────────────────────────────
# `hold = -1` значит «не уходить самому». Такт кончается ДЕЙСТВИЕМ игрока, а не
# секундомером (правило 2 наверху файла), и уведомление, гаснущее по своему
# таймеру, забрало бы подсказку у того, кто читает медленно.
func _say(big: String, small: String) -> void:
	_hush()
	_sfx(SFX_DONE if big == String((BEATS[BEATS.size() - 1] as Dictionary).get("big", "")) \
		else SFX_SAY, -8.0)
	# Команда — ЗАГОЛОВКОМ, пояснение — текстом. Слитые в один абзац, они читались
	# бы как одна длинная фраза, и «ВЕДИ ПАЛЬЦЕМ» потерялось бы в ней.
	_caption = PhonePush.spawn(_layer, small, "hleb", -1.0, _normaldo, big)
